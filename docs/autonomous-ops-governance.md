...
Outreach approval packets are handled by `scripts/sats-outreach-approval-agent.mjs`. Run `npm run ops:outreach-approval-plan` to list prospects eligible for contact approval, and use `node scripts/sats-outreach-approval-agent.mjs draft --prospects "<id,id>"` only for records already in `chairman-review`. After the matching outreach approval item is `approved-by-chairman`, use `node scripts/sats-outreach-approval-agent.mjs advance --approvalId "<approval-id>" --prospects "<id,id>" --transitionedAtUtc "<transitioned-at-utc>"` to move those records to `outreach-approved`. This drafts contact approval only; exact-sats invoices and payment instructions remain separate approval items.

Prospect response transitions are handled by `scripts/sats-prospect-response-agent.mjs`. Run `npm run ops:prospect-response-plan` after approved outreach exists. Use `node scripts/sats-prospect-response-agent.mjs record-contacted --prospect "<id>" --evidence "<contact-evidence-url-or-reference>" --channel "manual-dm-or-email" --contactedAtUtc "<contacted-at-utc>"` only after the approved message was actually sent and contact evidence is available. Use `node scripts/sats-prospect-response-agent.mjs record-invoice-request --prospect "<id>" --offer "<offer-id>" --evidence "<invoice-request-evidence-url-or-reference>" --confirmedCustomerRequestedInvoice true --requestedAtUtc "<requested-at-utc>"` only after the contacted prospect explicitly asks for an invoice. These commands record evidence only; they do not approve outreach, invoices, payment instructions, paid work, token grants, custody changes, or asset movement.
