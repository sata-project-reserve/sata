...
export function renderAuditIntakeComment(draft) {
  ...
  return [
    ...
    `- Inbound lead draft id: ${draft.inboundLeadDraft.id}`,
    `- Inbound lead status: ${draft.inboundLeadDraft.status}`,
    `- Offer: ${draft.invoiceDraft.offerId} ($${draft.invoiceDraft.usdPrice})`,
    `- Delivery draft: ${draft.deliveryDraft.title}`,
    '',
    'Record inbound lead after review:',
    '',
    '```sh',
    draft.inboundLeadDraft.recordFromIssueCommand,
    '```',
    '',
    'Expanded record command:',
    '',
    '```sh',
    draft.inboundLeadDraft.recordCommand,
    '```',
    '',
