...
- `npm run social:agent -- record-published --post <post-id> --postUrl "https://x.com/SATAReserve/status/<numeric-id>" --evidence "<live-post-screenshot-or-exported-text>" --publishedAtUtc "<published-at-utc>" --contentHash "<approved-content-sha256>"` records a manually published approved post and adds it to the monitoring log only when the submitted text and SHA-256 match the approved queue item.
